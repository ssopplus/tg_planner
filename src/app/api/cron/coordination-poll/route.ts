 ...
 * опрос за весь день.
 *
 * POLL_TIME стоит на десять минут раньше расписания крона (17:00): если
 * планировщик сработает чуть раньше срока, вызов всё равно попадёт в
 * разрешённый интервал, а не отлетит «ещё рано». Дублей это не создаёт — опрос за день один (уникальный
 * индекс user_id + poll_date), а уже отправленному проставлен message_id.
 */
const POLL_TIME = '16:50'
