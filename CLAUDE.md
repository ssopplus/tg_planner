...
**Связь с очередями Яндекс.Трекера** задаётся не в vault, а в настройках Mini App
(раздел «Очереди Трекера») и живёт в таблице `tracker_queue_links`. Одна очередь
может кормить несколько проектов: связка несёт необязательный фильтр по подстроке
заголовка, а связка без фильтра забирает остальные тикеты очереди. Подключить
очередь = добавить строку в настройках, **без правки кода, заметок и деплоя**.
Подробнее: [docs/yandex-tracker-sync.md](docs/yandex-tracker-sync.md).

**Списание координации.** По будням в 18:00 бот присылает опрос по очереди
`INTCOORD` (дейли и созвоны) и пишет ответ в Трекер через учёт времени —
единственная очередь, куда боту разрешено писать часы. `/coord [день]` открывает
экран дня: что списано, добавить запись, поправить или удалить существующую.
Меню команд Telegram регистрируется отдельно — `pnpm bot:commands`.
Подробнее: [docs/coordination-poll.md](docs/coordination-poll.md).
...
- `--dry` — ничего не писать (ни заметки, ни БД), только показать, что было бы сделано.
