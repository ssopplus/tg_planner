...
Источник истины для проектов — Obsidian-vault в [Документация/Проекты/](../../../Документация/Проекты/). Slug проекта = имя файла без `.md`. После того как репозиторий склонирован в `Личное/project/<slug>/` или `Vodohod/Projects/<slug>/`:

1. Создать заметку `Документация/Проекты/<категория>/<slug>.md` (frontmatter с `tags`, секции `# Заголовок`, `## Технологии`, `## Связанные проекты`).
2. Прогнать локальную синхронизацию: `pnpm sync:vault`. Скрипт идемпотентен, UPSERT по `(user_id, slug)`.
3. Прогнать прод-синхронизацию: `DATABASE_URL='<prod-neon-url>' pnpm sync:vault`. URL берётся из Vercel → Settings → Environment Variables.
4. Проверить в Mini App, что проект появился во вкладке «Проекты».

Скрипт ищет репо по slug в `Vodohod/Projects/` и `Личное/project/`, проставляет `repo_path` и `kind=dev`. Если репо нет — `kind=general`, путь пустой. Этого достаточно, чтобы потом сгенерировать AI-промт с правильным контекстом для dev-задач (Этап C плана).

Авто-синхронизация vault → tg-planer через obsidian-git + webhook — отдельный этап (D в плане), пока ручной запуск.
